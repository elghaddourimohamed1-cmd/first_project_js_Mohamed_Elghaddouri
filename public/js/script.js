    ...
    if (askUser === "3" || askUser === "LogIn" || askUser === "login" || askUser === "log in") {
        ...
        alert("No account found with that email");
            ...
            while (true) {
                ...
                === BANK DASHBOARD ===
                Your Current Balance: ${users.balance} DH
            1 - Logout
            2 - Withdraw Money
            3 - Deposit Money
            4 - Take a Loan
            ...
            `);
                ...
                if (dashboard === "1" || dashboard === "Logout" || dashboard === "logout" || dashboard === "log out") {
                    alert("Logging out From Your Bank Dashboard.\n Returning to the main menu.");
                    break;
                }   
                if (dashboard === "2" || dashboard === "Withdraw" || dashboard === "withdraw") {
                    let toWithdraw = prompt(`Enter amount to withdraw (Max available: ${users.balance} DH):`);
                    let withdrawAmount = parseFloat(toWithdraw);

                    if (!isNaN(withdrawAmount) && withdrawAmount > 0) {
                        if (amountToWithdraw <= users.balance) {
                            users.balance -= amountToWithdraw;
                            users.history.push(`Withdrew: -${amountToWithdraw} DH`);
                            alert(`Successfully withdrew ${amountToWithdraw} DH. New Balance: ${users.balance} DH`);
                        } else {
                ...
                if (dashboard === "3" || dashboard === "Deposit" || dashboard === "deposit") {
                    let toDeposit = prompt("Enter amount to deposit (Maximum limit: 1000 DH):");
                    let amountToDeposit = parseFloat(depositInput);

                    if (!isNaN(amountToDeposit) && amountToDeposit > 0) {
                        if (amountToDeposit <= 1000) {
                            users.balance += amountToDeposit;
                            users.history.push(`Deposited: +${amountToDeposit} DH`);
                            alert(`Successfully deposited ${amountToDeposit} DH. New Balance: ${users.balance} DH`);
                        } else {
                            alert("Transaction Denied: Deposit cannot exceed 1000 DH.");
                        }
                ...
                if (dashboard === "4" || dashboard === "Loan" || dashboard === "loan") {
                    let maximumToLoan = users.balance * 0.20; 
                    let inputLoan = prompt(`Enter loan amount (Max 20% of balance: ${maximumToLoan} DH):`);
                    let loanAmount = parseFloat(inputLoan);

                    if (!isNaN(loanAmount) && loanAmount > 0) {
                        if (loanAmount <= maximumToLoan) {
                            users.balance += loanAmount;
                            users.loanToBePaid += loanAmount; // Tracks how much needs to be paid back
                            users.history.push(`Took Loan: +${loanAmount} DH`);
                            alert(`Can you take a loan of ${loanAmount} DH to your account`);
                        } else {
                            alert(`Denied: You can only borrow up to ${maximumToLoan} DH (20% of your balance).`);
                        }
                ...
                if (dashboard === "5" || dashboard === "Invest" || dashboard === "invest") {
                    let investInput = prompt(`Enter amount to invest (Max available: ${users.balance} DH):`);
                    let investAmount = parseFloat(investInput);

                    if (!isNaN(investAmount) && investAmount > 0) {
                        if (investAmount <= users.balance) {
                            users.balance -= investAmount;
                            users.investedAmount = investAmount;
                            users.investReturnsReceived = 0;
                            users.history.push(`Invested: -${investAmount} DH`);
                            alert(`Successfully invested ${investAmount} DH.`);
                        } else {
                            alert("Transaction Denied: Insufficient funds to invest this amount.");
                        }
                    } else if (investInput !== null) {
                        alert("Invalid amount entered.");
                    }
                    continue;
                }
